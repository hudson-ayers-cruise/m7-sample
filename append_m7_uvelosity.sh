...
get_symbol_addr () {
	...
	printf "0x%s" $(echo ${tmp} | awk '{ print $2 }' | cut -d '+' -f1)
}
...
get_section_addr () {
	...
	if ! grep -q -w "${section}" "${map}"; then
		exit 1
	fi
	tmp=$(grep -w "${section}" "${map}")
	printf "0x%s" $(echo ${tmp} | awk '{ print $2 }')
}
...
# Very conservative while I test.
m7_bin_size=0x60000
