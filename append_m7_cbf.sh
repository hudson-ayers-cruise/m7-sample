...
# - take two images, program_image.s32 (IVT + QSPI params + headers)
#   and appcode.s32 (BL2 / BL31 / BL33)
# - program_image.s32: modify IVT header to set boot_target = M7
# - program_image.s32: modify IVT header to set the start address
# - program_image.s32: modify IVT header to set the new length
# - appcode.s32: add the M7 binary to the configured offset
# With these settings, bootROM copies the M7 bootloader and BL2
...
show_usage ()
{
	...
	echo "Parameters:"
	echo "    -i input IVT file, e.g. program-image.s32"
	echo "    -a input appcode file, e.g. appcode.s32"
	echo "    -b m7 binary file, e.g. m7.bin"
	echo "    -m m7 map file, e.g. m7.map"
	echo "    -o output IVT file (optional), If skip is used <input program_image file>.m7"
	echo "    -c output appcode file (optional), If skip is used <input appcode file>.m7"
	echo "    -e show expected M7 entry point (optional). -b and -m are optional when using -e"
	echo "    -h show this help"

	echo "Example"
	echo "./append_m7.sh -i program_image.s32 -a appcode.s32 -b build/m7.bin -m build/m7.map"
}


unset program_image appcode m7_file m7_map output_program_image output_appcode show_expected_ep
while getopts ":hi:a:b:m:o:c:e" input_params
do
	case $input_params in
		i) program_image="$OPTARG"
			;;
		a) appcode="$OPTARG"
			;;
		b) m7_file="$OPTARG"
		...
		m) m7_map="$OPTARG"
			;;
		o) output_program_image="$OPTARG"
			;;
		c) output_appcode="$OPTARG"
			;;
...
check_file "${program_image}"
check_file "${appcode}"

ivt_header_off=$(get_ivt_offset "${program_image}")
if [ -z "$ivt_header_off" ]
then
	exit 1
fi
...
app_header_off=$(get_u32_val "${program_image}" $((ivt_header_off + app_boot_header_off)))
...
ram_start_orig=$(get_u32_val "${program_image}" $((app_header_off + app_start_off)))
...
if [ -z "${output_program_image}" ]; then
	output="${program_image}.m7"
fi

if [ -z "${output_appcode}" ]; then
	output="${appcode}.m7"
fi
...
rm -f "${output_program_image}"
rm -f "${output_appcode}"
# write from input file until uboot_off
dd of="${output_program_image}" if="${program_image}" conv=notrunc seek=0 skip=0 count=$(hex2dec $uboot_off) status=none iflag=count_bytes

# update boot target. M7 boot_target -> 0x0
printf \\x00 | dd of="${output_program_image}" conv=notrunc seek=$(hex2dec $boot_target_off) status=none oflag=seek_bytes
...
dd of="${tmpfile}" if="${output_appcode}" count=4 skip=$(hex2dec $((app_entry_off))) status=none iflag=skip_bytes,count_bytes

# update entry point
int2bin $m7_bootloader_entry | dd of="${output_appcode}" bs=1 conv=notrunc seek=$(hex2dec $((app_entry_off))) status=none

# update Ram start
int2bin $ram_start | dd of="${output_appcode}" bs=1 conv=notrunc seek=$(hex2dec $((app_start_off))) status=none

# read the original app code size from IVT header (this is just the size copied by bootROM, aka just BL2)
blob_size=$(get_u32_val "${output_program_image}" $((app_header_off + app_size_off)))
...
int2bin $blob_size | dd of="${output_program_image}" bs=1 conv=notrunc seek=$(hex2dec $((app_header_off + app_size_off))) status=none

# write M7 bootloader -- right after the app header
dd of="${output_appcode}" if="${m7_file}" conv=notrunc seek=$(hex2dec $app_code_off) status=none oflag=seek_bytes
